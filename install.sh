...
check_token() {
  ...
  if [ "$USER_TOKEN" = "tukezgacor" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke Tukez${NC}"
    echo -e "${YELLOW}TELEGRAM : @rezyi${NC}"
    echo -e "${YELLOW}WHATSAPP : 6282114081878${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©reyzyi${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Tuxz      ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : reyzyi       ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Tuxz         ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Tuxz         ${NC}"
